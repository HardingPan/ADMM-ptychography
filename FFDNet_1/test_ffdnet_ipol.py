...
from FFDNet_1.utils import batch_psnr, normalize, init_logger_ipol, \
                variable_to_cv2_image, remove_dataparallel_wrapper, is_rgb

os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = "0"

def test_ffdnet(net,img,state_dict,**args):
    ...
    outimg = variable_to_cv2_image(outim)
    # outimg = (outimg - 0.2)/0.8
    outimg = outimg*(max-min) + min

    return outimg
def ffdnet(img,net,sigma,model_fn):
    # Parse arguments
    parser = argparse.ArgumentParser(description="FFDNet_Test")
    parser.add_argument('--add_noise', type=str, default="False")
    parser.add_argument("--input", type=str, default="./test/lely.png", \
                        help='path to input image')
    parser.add_argument("--suffix", type=str, default="", \
                        help='suffix to add to output name')
    parser.add_argument("--noise_sigma", type=float, default=sigma, \
    ...
    out = test_ffdnet(net, img, model_fn, **vars(argspar))
    return out
if __name__ == "__main__":
    ...
    PH = np.angle(A)
    print(np.max(PH))
    IMG=ffdnet(PH,net,30)
    print(np.max(IMG))
    IMG2 = ffdnet(abs(A),net,30)
    p =IMG2*np.exp(1j*IMG)
    cv2.imshow("xiangwei",np.angle(p))
    cv2.imshow("zhenfu",abs(IMG2))
    cv2.waitKey(0)